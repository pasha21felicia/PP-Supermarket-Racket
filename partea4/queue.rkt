...
(define (queue-empty? q)
    (if (and(equal? (queue-size-l q) 0) (equal? (queue-size-r q) 0))
      #t
      #f))
...
(define (enqueue x q)
  (cond
    ((queue-empty? q) (make-queue (stream-cons x (queue-left q)) (queue-right q) (+ (queue-size-l q) 1) (queue-size-r q)))
    ((equal? (queue-size-l q) (queue-size-r q)) (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (+ (queue-size-l q) (queue-size-r q) 1) 0))
    ((> (queue-size-l q) (queue-size-r q)) (make-queue (queue-left q) (cons x (queue-right q)) (queue-size-l q) (+ (queue-size-r q) 1)))
    ))

;(equal? (queue-size-l q) (queue-size-r q))
; TODO
...
(define (dequeue q)
  (cond
    ...
    ((equal? (queue-size-l q) (queue-size-r q)) (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '()  (- (+ (queue-size-l q) (queue-size-r q)) 1) 0))
    ))
